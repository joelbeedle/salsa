...
class SandboxSimulator final : public Test {
private:
  salsa::Sim* sim;
...
public:
  SandboxSimulator() {
    ...
    const auto behaviour_names = salsa::behaviour::Registry::get().
        behaviour_names();
  ...
  static void SetHeight(const float height) {
    sim_builder->setWorldHeight(height);
  }
  ...
  void UpdateUI() override {
    ...
    builder.addComponent(std::make_unique<BehaviorSettingsComponent>(sim))
        .addComponent(
            std::make_unique<VisualSettingsComponent>(
                draw_visual_range_, draw_targets_, sim))
        .addComponent(std::make_unique<DroneSettingsComponent>(sim))
        .addComponent(std::make_unique<SimulationSettingsComponent>(
            sim, pause, update_drone_count_, new_count, m_world, g_camera));
